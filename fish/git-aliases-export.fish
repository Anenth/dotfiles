...
alias gbda 'git branch --no-color --merged | command grep -vE "^(\*|\s*(master|develop|dev|recruiterbox_main)\s*\\$)" | command xargs -n 1 git branch -d'
...
alias gcmao 'git checkout main'
alias gcm 'gcmo & ggpur'
alias gcrm 'git checkout recruiterbox_main & ggpur'
alias gcma 'gcmao & ggpur'
